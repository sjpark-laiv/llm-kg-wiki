...
2. **`wiki/entity-list.md`**(허브)를 읽어 기존 엔티티·rel 인덱스에서 대상 문서의 고유명사·약어·`Entity ID`와 매칭되는 항목을 식별한다. 매칭이 불확실할 때만 `rg`·`Grep`으로 보조 검색한다. **`wiki/entity/*.md`** 에 정의된 실체가 본문에만 있고 **링크가 없으면** `[**표시명**](entity/<slug>.md)` 형태로 연결한다.
